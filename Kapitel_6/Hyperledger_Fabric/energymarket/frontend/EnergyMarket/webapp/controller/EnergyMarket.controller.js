sap.ui.define([
	...
	return Controller.extend("EnergyMarket.controller.EnergyMarket", {
				...
				success: function (data) {
					if (Array.isArray(JSON.parse(data))) {
						...
						JSON.parse(data).map(o => {
							const buyer = o.Record.buyerId ? o.Record.buyerId : "-";
							const offer = {
								"productName": o.Record.productName,
								"productDescription": o.Record.description,
								"productPrice": o.Record.price,
								"productBuyer": buyer,
								"productId": o.Key
							};
							offers.push(offer);
					...
					if (Array.isArray(JSON.parse(data))) {
						...
						JSON.parse(data).map(o => {
							const offer = {
								...
								"productDescription": o.Record.description,
								"productPrice": o.Record.price,
								"productOfferor": o.Record.offerorId,
								"productId": o.Key
							};
							offers.push(offer);
						});
		...
		onBtnCreateOffer: function () {
			...
			if (offer.productName && offer.productDescription && offer.productPrice) {
				...
				const payload = {
					...
					"offeror_id": this.currentUser.userId,
		...
		onBtnBuyOffer: function () {
			...
			if (offer.productOfferor === this.currentUser.userId) {
				return MessageToast.show(i18nModel.getText("notPossibleToBuyOwnOffer"), {
					duration: 2000
				});
			} else if (offer.productPrice > this.currentUser.accountBalance) {
			...
			const payload = {
				...
				"buyer_id": this.currentUser.userId
			};
			sap.ui.core.BusyIndicator.show();
			$.ajax({
				type: "Post",
